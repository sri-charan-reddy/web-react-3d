...
The Python backend (`run_dashboard.py`) is unchanged and still owns the
simulation engine and every `/api` route.
...
cd .. && python3 run_dashboard.py --ui react     # http://localhost:8080
```
...
Scripts: `npm run dev`, `build`, `preview`, `test`, `test:watch`, `typecheck`, `lint`.
